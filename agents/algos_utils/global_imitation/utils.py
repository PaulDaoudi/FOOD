...
def flatten_completed_trajs(
    completed_trajs: List[Dict[str, List[np.ndarray]]]
) -> List[Dict[str, List[np.ndarray]]]:
    """
    Flattens completed trajectories into a list of dictionaries.

    Args:
        completed_trajs (List[Dict[str, List[np.ndarray]]]): List of completed trajectories.

    Returns:
        List[Dict[str, List[np.ndarray]]]: Flattened list of trajectories.
    """    new_completed_trajs = []
...
class OrderedDefaultDict(OrderedDict):
    """
    An OrderedDict with default factory.
    """
...
class FixSizeOrderedDict(OrderedDefaultDict):
    """
    An OrderedDefaultDict with a fixed maximum size.
    """
...
def log_sum_exp(value: torch.Tensor, dim: int, keepdim: bool) -> torch.Tensor:
    """
    Numerically stable implementation of the log-sum-exp operation.
...
def obs_batch_normalize(
    obs_tnsr: torch.Tensor, 
    update_rms: bool, 
    rms_obj: RunningMeanStd
) -> torch.Tensor:
    """
    Normalizes a batch of observations using a running mean and standard deviation.

    Args:
        obs_tnsr (torch.Tensor): Batch of observations.
        update_rms (bool): Whether to update the running mean and standard deviation.
        rms_obj (RunningMeanStd): Running mean and standard deviation object.
...
class AddBias(nn.Module):
    """
    Module to add a learnable bias to the input.
    """
...
class RunningMeanStd:
    """
    Class for running mean and standard deviation.
    ...
    def update(self, x: np.ndarray) -> None:
        ...
        Args:
            x (np.ndarray): New batch of data.
    ...
    def update_from_moments(self, batch_mean: np.ndarray, batch_var: np.ndarray, batch_count: int) -> None:
        """
        Updates the running mean and variance using batch moments.

        Args:
            batch_mean (np.ndarray): Batch mean.
            batch_var (np.ndarray): Batch variance.
...
def update_mean_var_count_from_moments(
    ...
    """
    Updates mean, variance, and count using batch moments.
    ...
    Returns:
        Tuple[np.ndarray, np.ndarray, float]: Updated mean, variance, and count.
    """
...
def init(module: nn.Module, weight_init: Any, bias_init: Any, gain: float = 1) -> nn.Module:
    """
    Initializes the weights and biases of a module.

    Args:
        module (nn.Module): Module to initialize.
        weight_init (Any): Weight initialization function.
        bias_init (Any): Bias initialization function.
        gain (float, optional): Gain for the weight initialization. Defaults to 1.
...
def cleanup_log_dir(log_dir: str) -> None:
    """
    Cleans up the log directory by removing monitor files.
