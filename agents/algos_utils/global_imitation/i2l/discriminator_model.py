...
from on_policy_sim2real.algo_utils.global_imitation.i2l.misc.utils import obs_batch_normalize, log_sum_exp, \
    RunningMeanStd


class I2LDiscriminator(nn.Module):
    def __init__(self, ob_dim, ac_dim, hidden_dim, device):
        super(I2LDiscriminator, self).__init__()
    ...
    def update(self, ac_eval_fn, pq_buffer, rollouts, num_grad_steps):
        self.train()
        pqb_gen = pq_buffer.data_gen_finite((len(pq_buffer)//rollouts.num_processes)//num_grad_steps)
    ...
    def predict_batch_rewards(self, rollouts, imit_only=False):
