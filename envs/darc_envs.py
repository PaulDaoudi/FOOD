# Copy from https://github.com/google-research/google-research/blob/master/darc/darc_envs.py
...
import gym.spaces
import numpy as np
import on_policy_sim2real.envs.reacher_7dof.reacher_7dof as reacher_7dof
...
class HalfJoint(gym.Wrapper):
    """Wrapper that disables one coordinate of the action, setting it to 0."""

    def __init__(self, env, broken_joint, half_decay=0.5):
        super(HalfJoint, self).__init__(env)
        # Change dtype of observation to be float32
        self.observation_space = gym.spaces.Box(
            low=self.observation_space.low,
            high=self.observation_space.high,
    ...
    def step(self, action):
        action = action.copy()
        if self.broken_joint is not None:
            action[self.broken_joint] *= np.float32(self.half_decay)
        return super(HalfJoint, self).step(action)


class BrokenJoints(gym.Wrapper):
    """Wrapper that disables one coordinate of the action, setting it to 0."""

    def __init__(self, env, broken_joints):
        super(BrokenJoints, self).__init__(env)
        # Change dtype of observation to be float32
        self.observation_space = gym.spaces.Box(
            low=self.observation_space.low,
            high=self.observation_space.high,
    ...
    def step(self, action):
        ...
        return super(BrokenJoints, self).step(action)
